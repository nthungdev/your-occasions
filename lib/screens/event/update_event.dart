...
class UpdateEventScreenState extends State<UpdateEventScreen> {
  ...
  initState() {
    ...
    _originCategories = widget.event.categories;
    _selectCategoryName = widget.event.categories;
    ...
    if (widget.event.address != null) {
      ...
      ps.search(widget.event.address).then((value) {
        setState(() {
          _placeData = value;
          _showMap = true;
        });
  ...
  Future<void> _updateCategories() async {
    ...
    await ecc.bulkDelete(widget.event.id);
    await ecc.bulkInsert(_selectCategoryName, widget.event.id);
  }
  ...
  void _generateCategoryContent() {
    Categories.all.forEach((category) {
      if (_selectCategoryName.contains(category.name)) {
        _selectCategoryValue[category.name] = ValueNotifier<bool>(true);
      }
      else {
  ...
  Future<bool> _save() async {
    if (!_isSaving) {
      ...
      try {
        ...
        if(url != null) {
          await _eventController.update(widget.event.id, hostId: hostId, name: name, description: description, startTime: start, endTime: endDate, picture: url);
        }
        else {
          await _eventController.update(widget.event.id, hostId: hostId, name: name, description: description, startTime: start, endTime: endDate);
        }
        await _updateCategories();
        _isSaving = false;

        Event afterUpdate = await _eventController.getEvent(hostId: hostId, name: name, startTime: start);

        Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => EventDetailScreen(afterUpdate)));
        return true;
      }
      catch (e) {
        print(e);
